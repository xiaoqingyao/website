...
Deskpanel = function (me) {
    ...
    var desktopWrapper = "<div id='desktopWrapper'></div>";
    ...
    var desktopAppListener = "<div class='appListContainer' customacceptdrop='{index}' index='{index}' _olddisplay='block' >";
    var defaultIndex = 0,
    ...
    return me = {
        ...
        create: function () {
            me.box = $(desktopWrapper);
            me.desktopsContainer = $(desktopsContainer);
            me.createDesktopsContainer(defaultNum);
            me.box.append(me.desktopsContainer);
            me.box.css({ "left": "0px", "right": "0px" });
        ...
        bindEvent: function () {
            ...
            me.desktopsContainer.find(".appListContainer").each(function () {
                ...
                desk.sortable({
                    ...
                    stop: function (event, ui) {
                        ...
                        if (p.hasClass("dock_middle")) ui.item.removeAttr("style");
            ...
            $(window).resize(me.refresh1);
        },
        createDesktopsContainer: function (n) {
            if (n && n != 0) {
                for (var i = 1; i <= n; i++) {
                    me.desktopsContainer.append(me.addContainer(i))
                }
            }
        },
        ...
        addIcon: function (icon, idx) {
            if (icon) {
                if ($.isArray(icon)) {
                    $.each(icon, function () {
                        me.addIcon(this.valueOf(), idx);
                    });
                    return me;
                }
                var Icon = typeof icon == 'string' ? appIcon_t1(icon) : icon;
                me.Icon.push(Icon);
        ...
        refresh1: function () {
            var ww = $(window).width(),
				wh = $(window).height();
            me.width = ww - me.spaceRight - me.spaceLeft;
            me.height = wh - me.spaceTop - me.spaceBottom;
            var desktopContainer = me.desktopsContainer.find(".desktopContainer");
            var desktop_current = me.desktopsContainer.find(".desktop_current");

            var appContainer = desktopContainer.find(".appListContainer");
            var a = "";
            $("#zoomWallpaperGrid,#zoomWallpaper").width(ww).height(wh);
            ...
            desktop_current.css("left", "0");

            var r = me.row = ~ ~(me.height / 112);
	...
	return me ={
		 ...
		 addIcon:function(icon,idx){
			if(icon){
				...
				var Icon=typeof icon=='string'?appIcon_t2(icon):icon;
				if(idx!=undefined){
...
Navbar = function (me) {
    ...
    for (var i = 0; i < data.length; i++) {
        menu += "<a class=\"dock-item\" href=\"#\" id=\"desk_" + (i + 1) + "\" name=\"desk_" + (i + 1) + "\">" +
			"<img src=\"" + data[i].icon + "\" /><table height=\"20\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" align=\"center\" style=\"width:75px;margin-left:-10px;color:#eee;margin-top:3px;\">" +
			"<tr><td class=\"appButton_appName_table_left1\">" + data[i].name + "</td>" +
				"<td class=\"appButton_appName_table_right\" width=\"10\"></td></tr></table></a>";
    }
    var _box = "<div id='dock' class='dock'><div class='dock-container' style='wdith:700px;'>" + menu + "</div></div>";
    ...
    return me = {
        ...
        bindEvent: function () {
            $("#dock a").click(function () {
                ...
                me.bindSwitchDesktopAnimate(index, thisPage); 
                NavbarStyle(_this); 
            });
        },
        ...
        changeStyle: function () {
            ...
            $("#desk_" + thisPage).children("img").attr({ "width": '64px' }, { "height": '64px' });
            $("#desk_" + thisPage).css({"top":"-38px"});
        }
    }
} ();
...
Arrows = function(){
	var arrows_l = "<div id='arrows_l' class='arrows' style='display:none'><img src='images/arrows_l_3.png' /></div>";
	var arrows_r = "<div id='arrows_r' class='arrows' style='display:none'><img src='images/arrows_r_1.png' /></div>";
	return arrows = {
		init:function(){
			arrows.create();
			arrows.setPosition();
		...
		changeStyle:function(t,c){
			...
			$("#desk_" + c).children("img").attr({ "width": '45px' }, { "height": '45px' });
			...
			this_img = this_img +""+ png;
			$("#desk_" + t).children("img").attr("src", this_img);
			$("#desk_" + t).children("img").attr({ "width": '64px' }, { "height": '64px' });
...
BottomBar = function(me){
	...
	var bottonbarbg = "<div class='bottomBarBg' style='display:none;'></div>";
	var bottomBarBgTask = "<div class='bottomBarBgTask' style='display:none;'></div>";
...
appIcon_t1 = appIcon_t0.extend({		
	...
	create:function(fid){
		...
		var name_table = $("<table height=\"20\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" align=\"center\" style=\"width:85px\">"+
...
window.parent.f_addTabItem(id, url, title)
...
NavbarStyle = function (_this){
	...
	for(var i=1;i<=DATA.menu.length;i++){
		...
		if($.trim(temp_id)!=$.trim(id)){
			...
			$("#" + temp_id).children("img").attr({ "width": '50px' }, { "height": '50px' });
			$("#" + temp_id).css("top", "-25px");
		}
	}
	...
	if(is_thisImg!=-1){
		...
		_this.children("img").attr({ "width": '64px' }, { "height": '64px' });
		_this.css("top", "-39px");
	}
}
mouseStyle = function(){
