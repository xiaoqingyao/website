...
<div id="panel"><dl><dt>资质等级</dt><dd><input  class="validate[optional]"  type="text" value="<%=hsFieldValue["AptitudeLevel"]%>" style="width:140px;"  maxlength="50" name="AptitudeLevel" id="AptitudeLevel" /></dd></dl><dl><dt>企业名称</dt><dd><select  id="{#ID#}" name="CompanyID"><%foreach (object key in ctrManageEdit.FieldBind("K_U_CompanyName", "CompanyName", "ID", "").Keys){%><option  <%if(hsFieldValue["CompanyID"] != null && hsFieldValue["CompanyID"].ToString().Contains(key.ToString())) {Response.Write("selected='selected'");}%> value="<%= key.ToString()%>"><%=ctrManageEdit.FieldBind("K_U_CompanyName", "CompanyName", "ID", "")[key].ToString()%></option> <% } %> </select></dd></dl></div>
<div class="Submit" id="HQB_Operation_Button">
 <asp:Button ID="btnModelManageEdit" OnClick="Edit"  runat="server" />
 <input type="button" value="返回" onclick="back()" />
 </div>
</div>
 <asp:HiddenField ID="hdnFieldFromUrlParamValue" runat="server" Value="" /><asp:hiddenfield ID="hdnNodeCode" runat="server"/><asp:HiddenField Value="K_U_AptitudeCompany" ID="hdnTableName" runat="server" /><asp:HiddenField Value="AptitudeLevel||1,CompanyID||6," ID="hdnFieldValue" runat="server" /><asp:HiddenField ID="hdnModelID" Value="100000007425925" runat="server" /><input type="hidden" id="hdnModelAlias" value="天健资质" name="hdnModelAlias"/> 
