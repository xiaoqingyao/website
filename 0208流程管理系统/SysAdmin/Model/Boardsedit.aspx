...
<div id="panel"><dl><dt>姓名</dt><dd><input  class="validate[optional]"  type="text" value="<%=hsFieldValue["BoardName"]%>" style="width:200px;"  maxlength="30" name="BoardName" id="BoardName" /></dd></dl><dl><dt>职位</dt><dd><input  class="validate[optional]"  type="text" value="<%=hsFieldValue["BoardPost"]%>" style="width:200px;"  maxlength="100" name="BoardPost" id="BoardPost" /></dd></dl><%AllPower.Config.UploadConfig uploadobj = AllPower.Config.Upload.GetConfig(GetUploadImgPath); %><dl><dt>介绍</dt><dd><textarea  id="BoardIntroduction" name="BoardIntroduction"><%=hsFieldValue["BoardIntroduction"]%></textarea><script type="text/javascript">    var BoardIntroduction$$ckeditor$$obj = CKEDITOR.replace('BoardIntroduction', { language: '<%=Resources.Model.DateLang%>', height: '300px',width:'650px' }); </script></dd></dl><dl><dt>是否独立董事</dt><dd><input  type="radio" <%if(hsFieldValue["IsBoard"] != null && hsFieldValue["IsBoard"].ToString().Contains("1")) {Response.Write("checked='checked'");}%>  name="IsBoard" value="1" />是<input  type="radio" <%if(hsFieldValue["IsBoard"] != null && hsFieldValue["IsBoard"].ToString().Contains("0")) {Response.Write("checked='checked'");}%>  name="IsBoard" value="0" />否</dd></dl></div>
