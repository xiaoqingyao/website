...
<div class="container"id="_ListInfoListTable">
  <h4>位置： <a href="#">系统管理</a><a> ></a> <a href="#">模型管理布</a><a> ></a> <span class="breadcrumb_current" > 增加文章</span> </h4>
    ...
    <div  class="function">
    <asp:PlaceHolder ID="plParseModelLinkButton" runat="server"><input type="button" onclick='backOriginalUrl(<%=originalUrl %>)' value="返回" style=" display:<%=originalUrlDisplay%>;"/><%if(!string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){Response.Write("<input type=\"button\" value=\"添加\" onclick=\"location.href='Couponedit.aspx?action=add&BusinessID=" + ctrManageList.KeepParamValue["BusinessID"]  + "';\"  runat=\"server\" id=\"btnNew\" />");}%><%if(!string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" value="删除" onclick="return confirmSetAction('{e}{(IsDel)(1)}','确定删除选定记录至回收站？');"  runat="server" id="btnLogicDelete" /><%}%><%if(string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" id="btnDelete" onclick="return confirmSetAction('{d}','确定彻底删除选定记录？');"  runat="server"  value="删除" /><%}%><%if(ctrManageList.IsAllowFlow && !string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){%><input type="submit"  onclick="return setAction('HQB_PastFlowCheck');" value="通过审核"  runat="server"  id="btnPastFlowCheck" /><%}%><%if(ctrManageList.IsAllowFlow && !string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){%><input type="submit"  onclick="return setAction('HQB_CancelFlowCheck');" value="取消审核"  runat="server"  id="btnCancelFlowCheck" /><%}%><input type="submit" value="取消推荐"  runat="server" onclick="return setAction('{e}{(IsPub)(0)}');" /><input type="submit" value="推荐"  runat="server" onclick="return setAction('{e}{(IsPub)(1)}');" /></asp:PlaceHolder>
        <asp:PlaceHolder ID="plDParseModelLinkButton" runat="server"></asp:PlaceHolder>
    </div>
             <ul class="ulheader"><li  style="width:45px;"><input type="checkbox" name="SlectAll" id="SlectAll" /></li><li  style="width:300px;"><a href="javascript:sort('K_U_Coupon.title','3')">标题</a></li><li  style="width:80px;"><a href="javascript:sort('K_U_Coupon.AddMan','3')">添加人</a></li><li  style="width:80px;"><a href="javascript:sort('K_U_Coupon.DownCount','3')">下载次数</a></li><li  style="width:80px;"><a href="javascript:sort('K_U_Coupon.Starttime','3')">开始日期</a></li><li  style="width:80px;"><a href="javascript:sort('K_U_Coupon.EndTime','3')">结束日期</a></li><li  style="width:60px;">首页推荐</li><li  style="width:80px;" ><a href="javascript:sort('K_U_Coupon.Orders','3')">排序</a></li><li style="width:120px;" >操作</li> </ul>
          <div id="HQB_ListInfo" style=" padding:0; margin:0;">
        <asp:Repeater ID="rptListInfo" runat="server">
        <ItemTemplate>
         <ul class="ulheader ulbody"><li style="width:45px;"><input type="checkbox" name="_chkID" value="<%#Eval("ID") %>" /></li><li  title="<%#Eval("title") %> " isClip="1"  style="text-align:left;width:300px;"><%#Eval("title") %></li><li  title="<%#Eval("AddMan") %> " isClip="1"  style="text-align:left;width:80px;"><%#Eval("AddMan") %></li><li  style="text-align:center;width:80px;"><%#Eval("DownCount") %></li><li  style="text-align:center;width:80px;"><%#string.Format("{0:yyyy-MM-dd}",Eval("Starttime").ToString()) %></li><li  style="text-align:center;width:80px;"><%#string.Format("{0:yyyy-MM-dd}",Eval("EndTime").ToString()) %></li><li style="text-align:left; width:60px;"><a href="?action={e}{(IsPub)(1)}&ID=<%#Eval("[K_U_Coupon_ID]") %>" ><%#ctrManageList.ParseFieldValueToText("100000135826183",Eval("IsPub") )%></a></li><li  id="HQB_Orders_<%#Eval("ID") %>" style="width:80px;"><div style=" display:none;" ><img src="../image/loading.gif"/></div><span><input style="width:70px;" type="text" value="<%#Eval("Orders") %>" onblur="setOrders('K_U_Coupon','<%#Eval("ID") %>',this.value)"  CssClass="validate[required,custom[onlyNumber]]"/></span></li><li style="width:120px;" ><%if(!string.Equals(ctrManageList.IsDel,"1") &&  !string.Equals(ctrManageList.IsArchiving,"1")){%><input type="button"  id="btnEdit" onclick="<%#"location.href='" + "Couponedit.aspx?action=edit&ID=" + Eval("ID").ToString()%>&BusinessID=<%=ctrManageList.KeepParamValue["BusinessID"]%>'" value="修改"/>&nbsp;<%}%><%if(!string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" id="btnLogicDelete" onclick="return rptConfirmSetAction('<%#"{e}{(IsDel)(1)}{ID=" + Eval("ID").ToString() + "}"%>','确定删除当前记录到回收站？');"  value="删除" /><%}%>&nbsp;<%if(string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" id="btnDelete" onclick="return rptConfirmSetAction('<%#"{d}{(ID)(=)(" + Eval("ID").ToString() + ")}"%>','确定彻底删除当前记录？');"   value="删除" />&nbsp;<%}%></li></ul>
        </ItemTemplate>
        </asp:Repeater>
        </div>
   ...
   <asp:HiddenField ID="hdnNotSearchField" runat="server" value="" /><asp:HiddenField ID="hdnBackDeliverUrlParam" runat="server" value=""  /> <asp:HiddenField ID="hdnModelID" Value="100000010970354" runat="server" /><asp:HiddenField ID="hdnTableName" Value="K_U_Coupon" runat="server" />  <asp:HiddenField ID="hdnDeliverAndSearchUrlParam" runat="server"  Value="BusinessID"/> <input type="hidden" value="&BusinessID=<%=ctrManageList.KeepParamValue["BusinessID"]%>" id="HQB_Model_DeliverUrlParam" /><asp:HiddenField ID="hdnCustomCol" Value="|$[K_U_Coupon.ID]" runat="server" />   <asp:HiddenField ID="hdnForignTableCol" value="" runat="server" /><asp:HiddenField ID="hdnShowCol" value="K_U_Coupon.ID,K_U_Coupon.FlowState,K_U_Coupon.title,K_U_Coupon.AddMan,K_U_Coupon.DownCount,K_U_Coupon.Starttime,K_U_Coupon.EndTime,K_U_Coupon.IsPub,K_U_Coupon.Orders" runat="server" />
