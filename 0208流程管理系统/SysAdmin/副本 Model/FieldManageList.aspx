<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FieldManageList.aspx.cs"   Inherits="KingTop.Web.Admin.FieldManageList" %>
        ...
        <asp:Repeater ID="rptModelField" runat="server">
            <ItemTemplate>
                <ul class="ulheader ulbody" onmouseover="$(this).addClass('rowSelected')" onmouseout="$(this).removeClass('rowSelected')">
                    ...
                    <li style="width:40px; color:Red;"><%#KingTop.Common.Utils.ParseModelFieldState(Eval("IsSystemFiierd"), "")%></li>
                    <li  style="width:60px;">
                        <asp:LinkButton ID="LinkButton4"  OnCommand="SetIsEnable"  OnClientClick="setLogTitle(this)"  RecordID='<%#Eval("ID") %>' CommandArgument='<%#Eval("ID").ToString() + "|" + Eval("IsSearch").ToString()%>' CommandName="ISSEARCH" runat="server"><%#KingTop.Common.Utils.ParseIsOrNot(Eval("IsSearch"), "否")%></asp:LinkButton>&nbsp;
                        <a title="点击配置搜索相关属性" href="javascript:openConfig('1','<%#Eval("ID") %>')">设置</a>
                    </li>
                    <li style="width:60px;">
                        <asp:LinkButton ID="LinkButton3"  OnCommand="SetIsEnable"  OnClientClick="setLogTitle(this)"  RecordID='<%#Eval("ID") %>' CommandArgument='<%#Eval("ID").ToString() + "|" + Eval("IsListEnable").ToString()%>' CommandName="ISLIST" runat="server"><%#KingTop.Common.Utils.ParseIsOrNot(Eval("IsListEnable"), "否")%></asp:LinkButton>
                        &nbsp;<a title="点击配置列表列相关属性" href="javascript:openConfig('2','<%#Eval("ID") %>')">设置</a>
                    </li>
                    <li style="width:60px;">
                        <asp:LinkButton ID="LinkButton5"  OnCommand="SetIsEnable"  OnClientClick="setLogTitle(this)"  RecordID='<%#Eval("ID") %>' CommandArgument='<%#Eval("ID").ToString() + "|" + Eval("IsInputValue").ToString()%>' CommandName="ISEDIT" runat="server"><%#KingTop.Common.Utils.ParseIsOrNot(Eval("IsInputValue"), "否")%></asp:LinkButton>
                        &nbsp;<a title="点击配置编辑页相关属性" href="javascript:openConfig('3','<%#Eval("ID") %>')">设置</a>
                    </li>
                    <li style="width:40px;">
                    <asp:LinkButton ID="LinkButton2"  OnCommand="SetIsEnable"  OnClientClick="setLogTitle(this)"  RecordID='<%#Eval("ID") %>' CommandArgument='<%#Eval("ID").ToString() + "|" + Eval("IsEnable").ToString()%>' CommandName="STATE" runat="server"><%#KingTop.Common.Utils.ParseState(Eval("IsEnable"),"状态不明")%></asp:LinkButton></li>
                    <li style="width:40px;">
                    <asp:LinkButton ID="LinkButton1"  OnCommand="SetIsEnable"  OnClientClick="setLogTitle(this)"  RecordID='<%#Eval("ID") %>' CommandArgument='<%#Eval("ID").ToString() + "|" + Eval("IsRequired").ToString()%>' CommandName="REQUIRED" runat="server"><%#KingTop.Common.Utils.ParseIsOrNot(Eval("IsRequired"), "否")%></asp:LinkButton>
                   </li>
