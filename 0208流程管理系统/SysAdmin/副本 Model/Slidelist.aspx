...
<div class="container"id="_ListInfoListTable">
  <h4>位置： <%GetPageNav(NodeCode);%></h4>
    ...
    <div  class="function">
    <asp:PlaceHolder ID="plParseModelLinkButton" runat="server"><input type="button" onclick='backOriginalUrl(<%=originalUrl %>)' value="返回" style=" display:<%=originalUrlDisplay%>;"/><%if(!string.Equals(ctrManageList.IsArchiving,"1") && !string.Equals(ctrManageList.IsDel,"1")){%><input type="button" value="添加" Href="Slideedit.aspx?action=add"  EditUrlParam="" id="btnNew" /><%}%><%if(!string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" value="删除" onclick="return confirmSetAction(this,'{e}{(IsDel)(1)}','确定删除选定记录至回收站？');"  runat="server" id="btnDelete" /><%}%><%if(!string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" value="生成HTML" onclick="return confirmSetAction(this,'{h}','确定将选定记录生成HTML？');"  runat="server" id="btnCreateHtml" /><%}%></asp:PlaceHolder>
        <asp:PlaceHolder ID="plDParseModelLinkButton" runat="server"></asp:PlaceHolder>
    </div>
             <ul class="ulheader"><li  style="width:45px;"><input type="checkbox" name="SlectAll" id="SlectAll" /></li><li  style="width:304px;">Link Image</li><li  style="width:80px;"><a href="javascript:sort('K_U_Slide.Thumbnail','3')">Thumbnail</a></li><li  style="width:200px;">URL</li><li  style="width:55px;">是否启用</li><li  style="width:60px;" ><a href="javascript:sort('K_U_Slide.Orders','3')">排序</a></li><li style="width:100px;" >操作</li> </ul>
          <div id="HQB_ListInfo" style=" padding:0; margin:0;">
        <asp:Repeater ID="rptListInfo" runat="server">
        <ItemTemplate>
         <ul style="height:94px;" class="ulheader ulbody"><li style="width:45px;height:94px;"><input type="checkbox" name="_chkID" value="<%#Eval("ID") %>" /></li><li  style="text-align:left;width:304px;height:94px;"><img src='<%= GetUploadImgUrl()%><%#Eval("Link_Image") %>.gif' /></li><li  style="text-align:left;width:80px;height:94px;"><img src='<%= GetUploadImgUrl()%><%#Eval("Thumbnail") %>' /></li><li isClip="1"  style="text-align:left;width:200px;height:94px;"><%#Eval("Url") %></li><li  style="text-align:left;width:55px;height:94px;"><%#ctrManageList.ParseFieldValueToText("100011343715625",Eval("IsEnable") )%></li><li  id="HQB_Orders_<%#Eval("ID") %>" style="width:60px;height:94px;"><div style=" display:none;" ><img src="../image/loading.gif"/></div><span><input style="width:70px;" type="text" value="<%#Eval("Orders") %>" onblur="setOrders('K_U_Slide','<%#Eval("ID") %>',this.value)"  CssClass="validate[required,custom[onlyNumber]]"/></span></li><li style="width:100px;height:94px;" ><%if(!string.Equals(ctrManageList.IsDel,"1") &&  !string.Equals(ctrManageList.IsArchiving,"1")){%><input type="button"  BtnType="Edit"  Href='<%#"Slideedit.aspx?action=edit&ID=" + Eval("ID").ToString()%>' value="修改"/>&nbsp;<%}%><%if(!string.Equals(ctrManageList.IsDel,"1")){%><input type="submit" BtnType="Delete" onclick="return rptConfirmSetAction(this,'<%#"{e}{(IsDel)(1)}{ID=" + Eval("ID").ToString() + "}"%>','确定删除当前记录？');" RecordID="<%#Eval("ID")%>"  value="删除" /><%}%>&nbsp;</li></ul>
        </ItemTemplate>
        </asp:Repeater>
        </div>
        <ul class="page">
