...
<span style="color:#ff0000;" id="CheckTitleISRepeatMSG"></span></dd></dl><dl><dt>发布时间</dt><dd><input id="UpdateDate"  value="<%if(hsFieldValue["UpdateDate"].ToString().Equals("DateTime.Now.ToString('yyyy-MM-dd')")){Response.Write(DateTime.Now.ToString("yyyy-MM-dd"));}else{Response.Write(string.Format("{0:yyyy-MM-dd}",hsFieldValue["UpdateDate"]));}%>"   name="UpdateDate" style="width:150px;"  class="Wdate" type="text" onFocus="WdatePicker({lang:'<%=Resources.Model.DateLang%>',skin:'whyGreen',dateFmt:'yyyy-MM-dd'})"/></dd></dl><dl><dt>图片</dt><dd><input   type="text" value="<%=hsFieldValue["FirstPagePic"]%>" style="width:300px;"  maxlength="0" name="FirstPagePic" id="FirstPagePic" /> <input type='button' onclick="InputImages('theForm', 'FirstPagePic', 1, 'jpg|gif|png', 4096, '',180,100)" value = '上传图片' /></dd></dl><dl><dt>简介</dt><dd><textarea style="height:150px;width:680px;"   class="validate[optional]"  id="Intro"  name="Intro"><%=hsFieldValue["Intro"]%></textarea></dd></dl><%AllPower.Config.UploadConfig uploadobj = AllPower.Config.Upload.GetConfig(GetUploadImgPath); %><dl><dt>文章内容</dt><dd><textarea  id="Content" name="Content"><%=hsFieldValue["Content"]%></textarea><script type="text/javascript">var Content$$ckeditor$$obj = CKEDITOR.replace('Content', { linkUploadAllowedExtensions: '<%=uploadobj.UploadFilesType %>|pdf', nodeId: 1,language:'<%=Resources.Model.DateLang%>', watermark: false, height: '400px', toolbar: 'ContentFull', modelId: 1, flashUploadAllowedExtensions: '<%=uploadobj.UploadMediaType %>', width: '689px', imageUploadAllowedExtensions: '<%=uploadobj.UploadImageType %>', skin: 'blue', thumbnail: false, fileRecord: true, fieldName: 'Content', wordPic: false, flashUpload: true, imageUpload: true, linkUpload: true, foreground: false, moduleName: '' }); </script></dd></dl><div style="clear:left"></div></fieldset></div>
<div class="Submit" id="HQB_Operation_Button">
 <asp:Button ID="btnModelManageEdit" OnClick="Edit"  runat="server" />
 <input type="button" value="返回" onclick="back()" />
 </div>
</div>
 <asp:HiddenField ID="hdnFieldFromUrlParamValue" runat="server" Value="" /><asp:hiddenfield ID="hdnNodeCode" runat="server"/><asp:HiddenField Value="K_U_Article" ID="hdnTableName" runat="server" /><asp:HiddenField Value="Title||0,UpdateDate|DateTime.Now.ToString('yyyy-MM-dd')|10,FirstPagePic||11,Intro||2,Content||3," ID="hdnFieldValue" runat="server" /><asp:HiddenField ID="hdnModelID" Value="000000000132380" runat="server" /><input type="hidden" id="hdnModelAlias" value="文章模型" name="hdnModelAlias"/> 
