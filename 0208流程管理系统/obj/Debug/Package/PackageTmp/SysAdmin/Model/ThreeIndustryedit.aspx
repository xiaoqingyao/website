...
<div id="panel"><dl><dt>产业名称</dt><dd><input  class="validate[optional]"  type="text" value="<%=hsFieldValue["Name"]%>" style="width:200px;"  maxlength="50" name="Name" id="Name" /></dd></dl><dl><dt>产业图片</dt><dd><input id=""  type="hidden" value="<%=hsFieldValue[""]%>" id="IndustryImage"  name="" /><input   type="text" value="<%=hsFieldValue["IndustryImage"]%>" style="width:150px;"  maxlength="" name="IndustryImage" id="IndustryImage" /> <input type='button' onclick="InputImages('theForm', 'IndustryImage', 1, '', 10240, '',433,80)" value = '上传图片' /></dd></dl><%AllPower.Config.UploadConfig uploadobj = AllPower.Config.Upload.GetConfig(GetUploadImgPath); %><dl><dt>产业简介</dt><dd><textarea  id="IndustryIntro" name="IndustryIntro"><%=hsFieldValue["IndustryIntro"]%></textarea><script type="text/javascript">    var IndustryIntro$$ckeditor$$obj = CKEDITOR.replace('IndustryIntro', { language: '<%=Resources.Model.DateLang%>', height: '300px',width:'650px' }); </script></dd></dl></div>
<div class="Submit" id="HQB_Operation_Button">
 <asp:Button ID="btnModelManageEdit" OnClick="Edit"  runat="server" />
 <input type="button" value="返回" onclick="back()" />
 </div>
</div>
 <asp:HiddenField ID="hdnFieldFromUrlParamValue" runat="server" Value="" /><asp:hiddenfield ID="hdnNodeCode" runat="server"/><asp:HiddenField Value="K_U_ThreeIndustry" ID="hdnTableName" runat="server" /><asp:HiddenField Value="Name||1,IndustryImage||11,|0|8,IndustryIntro||3," ID="hdnFieldValue" runat="server" /><asp:HiddenField ID="hdnModelID" Value="100000005393757" runat="server" /><input type="hidden" id="hdnModelAlias" value="企业产业模型" name="hdnModelAlias"/> 
