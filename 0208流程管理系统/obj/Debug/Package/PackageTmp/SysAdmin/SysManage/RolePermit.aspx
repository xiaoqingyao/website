<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RolePermit.aspx.cs" Inherits="AllPower.WEB.SysAdmin.SysManage.RolePermit" %>
...
<head id="Head1" runat="server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function btnSaveOnClick() {
            ...
            //strUserPk = document.all("txtRoleCode").value;
            strUserPk = document.getElementById("txtRoleCode").value;
            if (strUserPk.length == 0) {
                alert({ msg: '未发现角色！', title: '提示信息' });
                return;
            }
            if (objChks.length == 0) {
            ...
            strXml = xmlLoadXMLHttp(null, "RolePermit.aspx?NodeId=" + NodeId, null, strXml, null);
            alert({ msg: '修改角色权限成功！', title: '提示信息' }); 
            return strXml;
        }
        function closeWin() {
...
<body>
    ...
    <div class="container">
        <h4>位置： <%GetPageNav(NodeCode); %> &gt; <a href="#">权限设置</a></h4>
    <div class="function">
        <input id="txtRoleCode" type="hidden" runat="server" />
        <input id="txtNodeId" type="hidden" runat="server" />
        <input id="btnOK" runat="server" type="button" value="保存"  onclick="btnSaveOnClick()" />
        <input type="button" name="Submit422" value="<%= Resources.Common.Back %>" onclick='location.href="RoleList.aspx<%=StrPageParams2("RoleList.aspx",Request.QueryString["NodeCode"]) %>";'>
    </div>
