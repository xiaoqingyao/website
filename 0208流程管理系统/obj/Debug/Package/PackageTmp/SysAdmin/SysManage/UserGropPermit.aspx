<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UserGropPermit.aspx.cs"
    Inherits="AllPower.WEB.SysAdmin.SysManage.UserGropPermit" %>
...
//            }
        ...
        function btnSaveOnClick() {
            var strXml = "", strID = "", strOprCode = "", strBindOperCode = "", strNodeCode = "", OperCode = "";
            ...
            strUserPk = document.getElementById("txtUserGropCode").value;
            if (strUserPk.length == 0) {
                alert({ msg: '未发现用户组！', title: '提示信息' });
                return;
            }
            if (objChks.length == 0) {
            ...
            for (var i = 0; i < objChks.length; i++) {
                ...
                OperCode = objChk.getAttribute("OperCode");

                strXml += "<c oprcode=\"" + strOprCode + "\" mdlcode=\"" + strMdlCode + "\" BindOperCode=\"" + strBindOperCode + "\" NodeCode=\"" + strNodeCode + "\" OperCode=\"" + OperCode + "\" />";
            }
            if (strXml.length == 0) {
                if (!confirm("您未选择任何权限，是否继续？")) { return; }
            }
            strXml = "<root type=\"save\" ><dataset userpk=\"" + strUserPk + "\" ><r>" + strXml + "</r></dataset></root>";
            strXml = xmlLoadXMLHttp(null, "UserGropPermit.aspx?NodeId=" + NodeId, null, strXml, null);
            alert({ msg: '修改用户组权限成功！', title: '提示信息' });          
            return strXml;
        }
        function closeWin() {
...
<body>
    ...
    <div class="container">
        <h4>
            位置： <%GetPageNav(NodeCode); %> &gt; <a href="#">权限设置</a>
        </h4>
        <div class="function">
          ...
          <input type="button" name="Submit422" value="<%= Resources.Common.Back %>" onclick='location.href="UserGropList.aspx<%=StrPageParams2("usergroplist.aspx",NodeCode) %>";'>
